...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        if order_exists:
            ...
            return HttpResponse(
                content=f'''
                Webhook received: {event["type"]} | SUCCESS: Verified order already in database
                ''',
                status=200,
            )
        else:
        ...
        return HttpResponse(
            content=f'''
            Webhook received: {event["type"]} | SUCCESS: Created order in webhook
            ''',
            status=200,
        )
